...
function getRanking(userUid, day, callbackFn) {
    var keyName = "cosmosBR:" + day; //cosmosBoosRanking
...
function setDayKiller(day, userUid, callbackFn) {
    var keyName = "cosmosBK:" + day; //cosmosBoosKiller
...
function setBossStatus(userUid, day, bossStatus, callbackFn) {
    var keyName = "cosmosBS:" + day; //cosmosBoosStatus
...
function getBossLevel(userUid, callbackFn) {
    redis.domain(userUid).s("cBossLevel").get(function (err, res) {
        if (err) {
            callbackFn(err);
        } else {
            if (res == null) {
                var sql = "SELECT * FROM serverData WHERE name='cBossLevel'";
                mysql.game(userUid).query(sql, function (err, res) {
                    if (err) callbackFn(err);
                        ...
                        if (res == null || res.length == 0) {
                            ...
                            redis.domain(userUid).s("cBossLevel").set(mBossLevel, function (err, res) {
                                //设置完成
                            });
...
function setBossLevel(userUid, bossLevel, callbackFn) {
    var sql = "UPDATE serverData SET value=" + mysql.escape(bossLevel) + " WHERE name='cBossLevel'";
    mysql.game(userUid).query(sql, function (err, res) {
        if (err || (res != null && res["affectedRows"] === 0)) {
            var insertSql = "INSERT INTO serverData SET ?";
            mysql.game(userUid).query(insertSql, {"name": "cBossLevel", "value": bossLevel}, function (err, res) {
                if (err) callbackFn(err);
                else {
                    redis.domain(userUid).s("cBossLevel").set(bossLevel, function (err, res) {
                        //设置完成
                    });
                    callbackFn(null, 1);
                }
...
function getUserTime(day, userUid, callbackFn) {
    var keyName = "cosmosBUT:" + day; //cosmosBoosUserTime
...
function getUserNameAndLevel(day, userUid, callbackFn) {
    var mKey = "cosmosB_NL:" + day;
    redis.domain(userUid).h(mKey).get(userUid, function (err, res) {
        if (err || res == undefined) {
            user.getUser(userUid, function (err, res) {
                if (err) callbackFn(err);
                else {
                    callbackFn(null, res["userName"] + "|" + res["lv"]);
                }
            });
        } else {
            callbackFn(err, res);
...
function setExpire(userUid, day) {
    ...
    redisClient.s("cosmosB_Str:" + day).expire(mTime);
    redisClient.s("cosmosB_NL:" + day).expire(mTime);
    redisClient.s("cosmosBCount:" + day).expire(mTime);
    redisClient.s("cosmosBBloody:" + day).expire(mTime);
    redisClient.s("cosmosBUT:" + day).expire(mTime);
    redisClient.s("cosmosBS:" + day).expire(mTime);
    redisClient.s("cosmosBK:" + day).expire(mTime);
    redisClient.s("cosmosBR:" + day).expire(mTime);
    redisClient.s("cosmosBHurt:" + day).expire(mTime);
    redisClient.s("cosmosBLast:" + day).expire(mTime);
}
