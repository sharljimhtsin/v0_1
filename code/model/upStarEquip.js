...
function getStarData(userUid, cb) {
    activityData.getActivityData(userUid, activityData.UPSTAREQUIP, function (err, res) {
        var obj;
        var jsonObj;
        if (res["arg"] == "") {
            obj = null;
...
function setStarData(userUid, data, cb) {
    ...
    activityData.updateActivityData(userUid, activityData.UPSTAREQUIP, mObj, cb);
}
    ...
    async.series([function (sCb) {
        ...
        for (var uid in starData) {
            ...
            switch (obj["type"]) {
                case "12":
                    addData[uid] = {
                        "attack": parseInt(obj["add"]) + parseInt(obj["attr"]),
                        "level": parseInt(obj["level"]) + 1
                    };
                    break;
                case "13":
                    addData[uid] = {
                        "defence": parseInt(obj["add"]) + parseInt(obj["attr"]),
                        "level": parseInt(obj["level"]) + 1
                    };
                    break;
                case "14":
                    addData[uid] = {
                        "hp": parseInt(obj["add"]) + parseInt(obj["attr"]),
                        "level": parseInt(obj["level"]) + 1
                    };
                    break;
            }
