/**
 * Created by xiazhengxin on 2017/2/13.
 */
...
function getStarData(userUid, cb) {
    activityData.getActivityData(userUid, activityData.UPSTARRECYCLE, function (err, res) {
        var obj;
        var jsonObj;
        if (res["arg"] == "") {
            obj = {};
...
function setStarData(userUid, data, cb) {
    ...
    activityData.updateActivityData(userUid, activityData.UPSTARRECYCLE, mObj, cb);
}
...
function getAddition(userUid, cb) {
    ...
    async.series([function (sCb) {
        ...
        for (var uid in starData) {
            ...
            switch (obj["type"]) {
                case "12":
                    addData[uid] = {"attack": parseInt(obj["add"]) + parseInt(obj["attr"])};
                    break;
                case "13":
                    addData[uid] = {"defence": parseInt(obj["add"]) + parseInt(obj["attr"])};
                    break;
                case "14":
                    addData[uid] = {"hp": parseInt(obj["add"]) + parseInt(obj["attr"])};
                    break;
            }
        }
        sCb();
