...
var ACTIVITY_CONFIG_NAME = "consumeRanking";
...
function addRecord(userUid,pay,callbackFn){
    ...
    async.series([
        ...
        function(cb) {
            getConfig(userUid, function(err, res){
                if (err || res == null) {
                    ...
                    stats.events(userUid, "127.0.0.1", null, mongoStats.CONSUMERANKING1);
                }
            });
            activityData.getActivityData(userUid, activityData.PRACTICE_CONSUME, function(err, res){
                if(err){
                    cb(err);
                    return;
                }
        ...
        function(cb) {
            ...
            //redis[isAll?"loginFromUserUid":"domain"](userUid)
            redis[isAll ? "loginFromUserUid" : "domain"](userUid).z("consume:" + key).add(number, userUid, function (err, res) {
                data["data"] = pay-0;
                cb(err,res);
            });
        },
        function(cb){
            activityData.updateActivityData(userUid,activityData.PRACTICE_CONSUME,data,cb);
...
//排行榜
function getTopList(userUid, key,isAll, callbackFn){
    redis[isAll?"loginFromUserUid":"domain"](userUid).z("consume:"+key).revrange(0 ,9 ,"WITHSCORES",function(err, res){
        var topList = [];
...
function getTop(userUid, key,isAll, callbackFn){
    redis[isAll?"loginFromUserUid":"domain"](userUid).z("consume:"+key).revrank(userUid,callbackFn);
}
//充值数
function getNumber(userUid, key,isAll, callbackFn){
    redis[isAll?"loginFromUserUid":"domain"](userUid).z("consume:"+key).score(userUid,function(err,res){
...
function setRewardStatus(userUid,callbackFn){
    activityData.updateActivityData(userUid,activityData.PRACTICE_CONSUME,{"status":1,"statusTime":jutil.now()},callbackFn);
}
//获取领取奖励状态
...
function setAnalytic(userUid, sTime, config, callbackFn){
    if(parseInt(config["isAll"])){
        ...
        async.series([
            ...
            function(cb){
                gsData.addGSDataInfo(userUid, "consumeRanking", sTime, {"data":JSON.stringify(topList), "status":1}, cb);
            }
        ], callbackFn);
    } else {
        callbackFn(null);
...
exports.getRewardData = getRewardData;//获取奖励数据
