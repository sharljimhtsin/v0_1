...
function addGroupPurchaseUser(userUid, issueId, number, callbackFn) {
    var insertSql = "INSERT INTO gsGroupPurchaseUser2 SET ?";
            ...
            redis.loginFromUserUid(userUid).s(_getGSRedisKey(issueId, "buyCount")).incrby(number, function (err, res) {//全服购买数
                if (err) callbackFn(err);
                    ...
                    redis.loginFromUserUid(userUid).s(_getGSRedisKey(issueId, "buyCount")).get(function(err,res){// 更新当前购买量
                        gsData.updateGSDataInfo(userUid, gsData.GS_GROUPPURCHASE2, issueId, {"data": res}, function (err, res) {});
                    })
...
function groupPurchaseSendLastReward(userUid,callbackFn) {
    ...
    async.series([
        ...
        function (cb) {
            gsData.getActivityConfig(userUid, gsData.GS_GROUPPURCHASE2, function (err, res) {
                if (err) cb(err);
                else {
                    activityEtime = res[5];
        ...
        function (cb) {
            gsData.getGSDataStatus1(userUid, gsData.GS_GROUPPURCHASE2, function (err, res) {
                issueId = res - 0;
                if (issueId > 0) {
                    cb(null);
                } else {
        ...
        function (cb) {
            if (jutil.now() - 0 > activityEtime - 0) {//结束活动
                gsData.updateGSDataInfo(userUid, gsData.GS_GROUPPURCHASE2, issueId, {"status": 2}, function (err, res) {
...
function _groupPurchaseRunSendRewardList(userUid, issueId, rankIndex, rankCount, rankReward) {
    redis.loginFromUserUid(userUid).s(_getGSRedisKey(issueId, rankIndex + ":sendsuccess:batch")).setnx(1, function (err, res) {
        if (err || res == 0) {//正在发
            ...
            async.series([
                function (cb) {
                    var sql = "SELECT distinct userUid FROM `gsGroupPurchaseUser2` WHERE issueId=" + mysql.escape(issueId);
...
function _getGSRedisKey(issueId,key){
    return gsData.getGSRedisKey(gsData.GS_GROUPPURCHASE2,issueId,key);
}
