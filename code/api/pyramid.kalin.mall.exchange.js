...
var TAG = "pyramid.kalin.mall.exchange";
    ...
    async.series([
        function (cb) {
            ...
            pyramid.checkIfEnough(userUid, {
                ...
                if (err) {
                    ...
                    modelUtil.addDropItemToDB(costId, shopItem["cost"] * buyCount * -1, userUid, 1, 1, function (err, res) {
                        mongoStats.expendStats(costId, userUid, "127.0.0.1", null, mongoStats.PYRAMID21, shopItem["cost"] * buyCount);
                        returnData.push(res);
                        cb(err);
                    });
                } else {
            ...
            modelUtil.addDropItemToDB(mItemId, buyCount * shopItem["count"], userUid, false, 1, function (err, res) {
                switch (mItemId) {
                    case "ingot":
                        mongoStats.dropStats(mItemId, userUid, "127.0.0.1", null, mongoStats.PYRAMID5, buyCount * shopItem["count"]);
                        break;
                    case "gold":
                        mongoStats.dropStats(mItemId, userUid, "127.0.0.1", null, mongoStats.PYRAMID11, buyCount * shopItem["count"]);
                        break;
                    default:
                        mongoStats.dropStats(mItemId, userUid, "127.0.0.1", null, mongoStats.PYRAMID17, buyCount * shopItem["count"]);
                }
                buyResult = res;
                cb();
            });
