...
function start(postData, response, query) {
    if (jutil.postCheck(postData,"itemId") == false) {
        response.echo("box.open",jutil.errorInfo("postError"));
        return;
    }
    ...
    async.series([
        ...
        function(cb) { // 掉落逻辑
            ...
            var mPoint = 0;
            if (gBoxData != null) {
                if (gBoxData["b" + itemId1] != null && gBoxData["b" + itemId1] - 0 > 0) {
                    pay1 = true;
                    updateBoxData["b" + itemId1] = gBoxData["b" + itemId1] - 1;
                }
                if (gBoxData["b" + itemId2] != null && gBoxData["b" + itemId2] - 0 > 0) {
                    pay2 = true;
                    updateBoxData["b" + itemId2] = gBoxData["b" + itemId2] - 1;
                }
                mPoint = gBoxData["point"] - 0;
            }
        ...
        function(cb) { //更新道具数量1
            itemModel.updateItem(userUid, itemId1, -1, function(err,res) {
                resultItem1 = res;
        ...
        function(cb) { //更新道具数量2
            itemModel.updateItem(userUid, itemId2, -1, function(err,res) {
                resultItem2 = res;
        ...
        function(cb) { // 成就数据更新
            ...
            if (itemConfig && itemConfig["star"] == 4) {
                achievement.boxOpen(userUid, 1, function(){
                    cb(null);
                });
            } else {
                cb(null);
        ...
        if (err) response.echo("box.open", jutil.errorInfo(err));
        else {
            timeLimitActivityReward.itemUsed(userUid, itemId1, 1, function(){
                response.echo("box.open", {"boxData":gRandomItem,"itemData":[resultItem1,resultItem2]});
            });
        }
    });
