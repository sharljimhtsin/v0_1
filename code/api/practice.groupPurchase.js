...
function start(postData, response, query) {
    if (jutil.postCheck(postData, "action") == false) {
        response.echo("practice.groupPurchase", jutil.errorInfo("postError"));
        return;
    }
    ...
    async.series([
        function (cb) { // 活动配制
            groupPurchase.getConfig(userUid, groupPurchase.GROUPPURCHASE, function (err, res) {
                if(err){
                    cb(err);
                } else {
                    sTime = res[0];
        ...
        function(cb) {//全服购买数
            ...
            redis[rk](userUid).s(groupPurchase.GROUPPURCHASE+":buyCount:"+key).get(function(err, res){
                if(err){
                    cb(err);
                } else {
        ...
        function(cb) {//用户购买数
            var rk = isAll ? (isAll == 2 ? "loginFromUserUid" : "difficulty") : "domain";//210
            redis[rk](userUid).h(groupPurchase.GROUPPURCHASE+":userBuyCount:"+key).get(userUid,function(err, res){
                if(err){
                    cb(err);
                } else {
        ...
        function(cb) {
            if (action == "buy") {// 只有购买才触发
                groupPurchase.groupPurchaseSendReward(userUid, buyCount, groupPurchase.GROUPPURCHASE, "buy", function (err, res) {
                    cb(null);
                });
            }else{
                cb(null);
        ...
        if (err) {
            response.echo("practice.groupPurchase", jutil.errorInfo(err));
        } else {
            response.echo("practice.groupPurchase",returnValue);
        }
    })
}
...
function Buy(userUid, count, activityConfig, callbackFn) {
    ...
    async.series([
        ...
        function (cb) {// 保存购买记录
            groupPurchase.addGroupPurchaseUser(userUid, groupPurchase.GROUPPURCHASE, itemCount, cb)
        }
    ], function (err, res) {
