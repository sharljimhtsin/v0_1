        ...
        function (sCb) {//判断是否达到可以打的条件
            judgeAchieveConditions(userUid, activityMapId, function (err, res) {
                if (err) sCb(err, null);
                    ...
                    //TODO: 根据 activityMapId 分支
                    var list = {"getItem_easy": 1, "getItem_normal": 2, "getItem_hard": 3, "getGold_hard": 4};//hard-coded
                    stats.recordWithLevelIndex(list[activityMapId], [mongoStats.activeMapNew1, mongoStats.activeMapNew2, mongoStats.activeMapNew3, mongoStats.activeMapNew4], function (tag) {
...
function getActivityConfigItem(userUid, id) {
    var configData = configManager.createConfig(userUid);
    var activityConfig;
    var activityConfigDefault = configData.getConfig("activityMap");
    var activityConfigLocal = configData.getConfig("activityMap" + "_" + lang);
    if (activityConfigLocal) {
        activityConfig = activityConfigLocal;
    } else {
        activityConfig = activityConfigDefault;
    }
