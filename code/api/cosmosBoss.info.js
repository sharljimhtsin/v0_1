    ...
    async.series([
        function(cb) {
            cosmosBoss.getBossStatus(userUid, gDay, function(err, res) {
                if (err) {
                    console.log("cosmosBoss.getBossStatus", err.stack);
                    cb("dbError");
                } else {
                    gBossStatus = res;
                    cb(null);
...
//                cb("outTime");
                ...
                cosmosBoss.getRankStr(userUid, gDay, function(err,res) {
                    if (err) {
                        console.log("cosmosBoss.getRankStr", err.stack);
                        cb("dbError");
                    } else {
                        echoStr = res;
                        cb(null);
        ...
        function(cb) { //取排名和伤血量
            cosmosBoss.getUserIndex(userUid, gDay, function(err, res) {
                if (err) {
                    console.log("cosmosBoss.getUserIndex", err.stack);
                    cb("dbError");
                } else {
                    totleHurt = res[1] || 0;
                    scoreValue = res[0] || 0;
...
//        },
        function(cb) { //取攻击次数
            cosmosBoss.getAttackCount(gDay, userUid, function(err, res) {
                if (err) {
        ...
        if (err) {
            console.log("cosmosBoss.info", err, err.stack);
            response.echo("cosmosBoss.info", jutil.errorInfo(err));
        }
        else {
            var mStr = '{"info":' + echoStr + ',"attackCount":' + attackCount + ',"scoreValue":' + scoreValue + ',"totleHurt":' + totleHurt + '}';
            response.echoString("cosmosBoss.info", mStr);
        }
    });
}
...
function getRealTimeInfoString(day, userUid,callbackFn) {
    cosmosBoss.getRealTimeInfoString(userUid, day, function(err, res) {
        if (res != null) callbackFn(null, res);
            ...
            realTimeInfo(day, userUid,function(err, res) {
                ...
                cosmosBoss.setRealTimeInfoString(userUid, day, resStr, function(){
...
function realTimeInfo(day, userUid,callbackFn) {
    ...
    async.series([
        function(cb) { //取排行
            cosmosBoss.getRanking(userUid, day, function(err,res) {
                if (err) {
                    cb(null);
                } else {
        ...
        function(cb){//取战斗开始的时间
            cosmosBoss.getUserTime(day, userUid, function(err, res) {
        ...
        function(cb) { //取用户名字
            async.forEach(mRanking, function(item, forCb) {
                ...
                cosmosBoss.getUserNameAndLevel(day, mUserUid, function(err, res) {
                    if (err) forCb(null);
                    else {
        ...
        function(cb) { //取boss伤害值
            cosmosBoss.getAllHurt(userUid, day, function(err, res) {
                bossHurt = res || 0;
                cb(null);
            });
        },
        function(cb) { //取boss总血量
            cosmosBoss.getBossLevel(userUid, function(err, res) {
                var bossLevel = 0;
                if (res == null) bossLevel = configData.g("cosmosBoss")("LevelInit")() - 0;
                else bossLevel = res - 0;
                bossHP = bossLevel * (configData.g("cosmosBoss")("hpPerLevel")() - 0);
                cb(null);
            });
        },
        function(cb) { //取攻击者列表
            cosmosBoss.getLastAttack(userUid,day, function(err, res) {
