...
exports.start = function(postData, response, query){
    ...
    if (jutil.postCheck(postData,"pay") == false) {
        response.echo("dailyCumulativeConsume.reward",jutil.errorInfo("postError"));
        return;
    }
    ...
    async.series([
        ...
        function (cb) {
            dailyConsume.updateRewardData(userUid, payIndex, false, sTime, function(err){
            ...
            activityConfig.getConfig(userUid, "dailyTotalConsume", function (err, res) {
                ...
                stats.recordWithLevel(pay, res[2]["list"], true, "pay", "", [mongoStats.DAILYTOTALCONSUME1, mongoStats.DAILYTOTALCONSUME2, mongoStats.DAILYTOTALCONSUME3, mongoStats.DAILYTOTALCONSUME4, mongoStats.DAILYTOTALCONSUME5, mongoStats.DAILYTOTALCONSUME6, mongoStats.DAILYTOTALCONSUME7, mongoStats.DAILYTOTALCONSUME8, mongoStats.DAILYTOTALCONSUME9, mongoStats.DAILYTOTALCONSUME10], function (tag) {
        ...
        if (err) {
            response.echo("dailyCumulativeConsume.reward",  jutil.errorInfo(err));
        } else {
            response.echo("dailyCumulativeConsume.reward",  rtnArr);
        }
    });
};
...
function __rwHandler(userUid, id, count, cb) {
    ...
    mongoStats.dropStats(id, userUid, 0, null, mongoStats.DAILY_CUMULATIVE_CONSUME, count);
    switch (id) {
        default:
