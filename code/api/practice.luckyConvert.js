        ...
        default:
            async.series([function(cb) {// 取配置
                ...
                luckyConvert.getUserData(userUid,sTime,false,function(err,res){//取用户数据
                    if(err||res["arg"] == null || res["arg"]["shopList"] == undefined)cb("dbError");
                    else {
                        userData = res;
                        ...
                        if(kind == 1){//手动刷新
                            while(list.length < 7){//需求：1,10,100
                                ...
                                for(var i in shopList){
                                    p += shopList[i]["prob"] - 0;
                                    if (randomRate <= p) {
                                        list.push({"id":shopList[i]["id"],"count":shopList[i]["count"],"type":shopList[i]["type"],"cost":shopList[i]["cost"]});
                                        break;
                                    }
                                }
                            }
