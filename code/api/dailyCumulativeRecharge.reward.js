...
exports.start = function(postData, response, query){
    ...
    if (jutil.postCheck(postData,"pay") == false) {
        response.echo("dailyCumulativeRecharge.reward",jutil.errorInfo("postError"));
        return;
    }
    ...
    async.series([
        ...
        // 取保存的奖励数据
        function(cb) {
            dailyCumulativeRecharge.getRewardData(userUid, sTime, function(err, res){
                if (err) cb(err);
                else {
                    rewardData = res || {};
                    cb(null);
        ...
        function (cb) {
            dailyCumulativeRecharge.updateRewardData(userUid, payIndex, false, sTime, function(err){
            ...
            activityConfig.getConfig(userUid, "dailyTotalRecharge", function (err, res) {
                ...
                stats.recordWithLevel(pay, res[2]["list"], true, "pay", "", [mongoStats.DAILYTOTALCHARGE1, mongoStats.DAILYTOTALCHARGE2, mongoStats.DAILYTOTALCHARGE3, mongoStats.DAILYTOTALCHARGE4, mongoStats.DAILYTOTALCHARGE5, mongoStats.DAILYTOTALCHARGE6, mongoStats.DAILYTOTALCHARGE7, mongoStats.DAILYTOTALCHARGE8, mongoStats.DAILYTOTALCHARGE9, mongoStats.DAILYTOTALCHARGE10], function (tag) {
                    stats.events(userUid, "127.0.0.1", null, tag);
                });
            });
            stats.dropStats("ingot",userUid,"127.0.0.1",null,mongoStats.DAILYTOTALCHARGE_count,pay);
        },
        ...
        if (err) {
            response.echo("dailyCumulativeRecharge.reward",  jutil.errorInfo(err));
        } else {
            response.echo("dailyCumulativeRecharge.reward",  rtnArr);
        }
    });
};
...
function __rwHandler(userUid, id, count, cb) {
    ...
    mongoStats.dropStats(id, userUid, 0, null, mongoStats.DAILY_CUMULATIVE_RECHARGE, count);
    switch (id) {
        default:
