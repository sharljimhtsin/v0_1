...
exports.start = function(postData, response, query){
    ...
    if (jutil.postCheck(postData,"pay") == false) {
        response.echo("oneRecharge.reward",jutil.errorInfo("postError"));
        return;
    }
    ...
    async.series([
        ...
        // 取保存的奖励数据
        function(cb) {
            practiceOneRecharge.getRewardData(userUid, sTime, function(err, res){
                if (err) cb(err);
                else {
                    rewardData = res || {};
                    cb(null);
        ...
        // 更新奖励数据
        function (cb) {
            practiceOneRecharge.updateRewardData(userUid, pay, false, timeLimit, sTime, function(err){
                if (err) {
                    console.error(err);
                    cb(err);
                } else {
        ...
        if (err) {
            response.echo("oneRecharge.reward",  jutil.errorInfo(err));
        } else {
            response.echo("oneRecharge.reward",  rtnArr);
        }
    });
};
...
function __rwHandler(userUid, id, count, cb) {
    ...
    mongoStats.dropStats(id, userUid, 0, null, mongoStats.ONERECHARGE, count);
    switch (id) {
        default:
