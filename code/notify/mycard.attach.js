...
function start(postData, response, query) {
    ...
    if (jutil.postCheck(postData, "MyCardTradeNo", "token", "order_sn", "user_id") == false) {
        response.end(JSON.stringify({"ret": -7, "msg": "參數錯誤"}), "utf-8");
        return;
    }
    var MyCardTradeNo = postData["MyCardTradeNo"];
    var token = postData["token"];
    var order_sn = postData["order_sn"];
    var user_id = postData["user_id"];
    var country = platformConfig[NAME]["country"];
    response.writeHead(200, {'Content-Type': 'text/plain', "charset": "utf-8"});
    redis.login(country).h("myCardOrder").set(order_sn.substr(0, 20), order_sn + "_" + user_id);
