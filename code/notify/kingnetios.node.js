...
function start(postData, response, query) {
    ...
    console.log("kingnetiosData......." + JSON.stringify(postData));
    ...
    if(sig != md5Sign) { //MD5校验成功
        ...
        if(postData["pay_ref"] != "kingnet_tw" && postData["pay_ref"] != "pay_admin") {
            order.updateOrder(role_id, stringA[0],"kingnetios",user_id,number,amount,1,"",stringA[1],JSON.stringify(postData), function(err, res) {
                if (res == 1){
            ...
            var _udid;
            var _pUserId;
            var country = platformConfig["kingnetios"]["country"];
            async.series([
                function(cb) {
