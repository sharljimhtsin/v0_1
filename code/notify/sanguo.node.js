...
function start(postData, response, query) {
    ...
    console.log("sanGuoData......." + JSON.stringify(postData));
    ...
    var payConfig = platformConfig[platform_id]["pay"]; //密钥
    ...
    if (payConfig.hasOwnProperty(product_id)) {
        var shouldPay = payConfig[product_id] - 0;
        if (shouldPay != amount) {
            console.log("支付金额驗證失敗");
            response.end(JSON.stringify({"ret": -9, "msg": "支付金额验证失败"}), "utf-8");
            return;
        }
    } else {
        console.log("支付金额驗證失敗");
        ...
        async.series([
            function (cb) {//过滤订单号
                ...
                mysql.game(userUid).query(sql, function (err, res) {
                    if (err) {
                        console.log("userUid: " + userUid + "notExist!");
                        cb("dbError");
                    }
