...
function start(postData, response, query) {
    ...
    var resource = "1124131"; //私钥
    var key = "kingnetpay.kingnet.com"; //公钥
    if (jutil.postCheck(postData, "ts", "sig", "user_id", "sid", "number", "amount", "role_id", "order_id") == false) {
    ...
    if (sig != md5Sign) { //MD5校验成功
        ...
        if (postData["pay_ref"] != "kingnet_os" && postData["pay_ref"] != "pay_admin") {
            order.updateOrder(stringA[0], stringA[3], platformName, user_id, number, amount, 1, ts, stringA[2], JSON.stringify(postData), function (err, res) {
                if (res == 1) {
