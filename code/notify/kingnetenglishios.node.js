...
function start(postData, response, query) {
    ...
    var platformName = "kingnetenglish";//非kingnetenglishios
    ...
    if(sig != md5Sign) { //MD5校验成功
        ...
        if(postData["pay_ref"] != "kingnet_os" && postData["pay_ref"] != "pay_admin") {
                ...
                platformName = "kingnetenglishios_googleplay_tw"; // a走套餐配制文件( gl储值走套餐配制文件pay.json )
                ...
                mysql.game(role_id).query(sql,newData,function(err,res) {
                    if (err) {console.log("更新订单状态failed!");cb("dbError");}
                    else {
