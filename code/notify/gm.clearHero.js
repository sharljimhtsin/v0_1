...
function start(postData, response, query) {
    var NAME = "gm";
    fs.appendFile('payOrder.log', NAME + "clearHero:" + jutil.now() + " | " + JSON.stringify(postData) + "\n" + JSON.stringify(query) + "\n", 'utf8');
    console.log(NAME + "...POST...." + JSON.stringify(postData));
    console.log(NAME + "...GET...." + JSON.stringify(query));
    if (jutil.postCheck(query, "user", "server_id", "sign") == false) {
    ...
    var signStr = server_id + "" + userId + "" + key;
    ...
    if (md5Sign == sign) {
        ...
        async.series([function (cb) {
            ...
            hero.delAllHero(userUid, cb);
        }, function (cb) {
            modelUtil.delData("heroSoul", userUid, cb);
        }, function (cb) {
            async.timesSeries(8, function (i, nextCb) {
                formation.removeFormation(userUid, i, nextCb);
            }, cb);
        }, function (cb) {
            ...
            if (err) {
                console.log(err);
                response.end(JSON.stringify({"errcode": "4", "msg": "删除失败"}), "utf-8");
            } else {
                response.end(JSON.stringify({"errcode": "0", "msg": "删除成功"}), "utf-8");
            }
        });
    } else {
